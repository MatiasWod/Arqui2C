...
section  .text
        global print_primes             ; Our program starts from 'main'.
...
print_primes:
        enter 0,0 ;con esta instruccion, armo el stack frame
        push ebx ; pusheo ebx para luego popearlo, ya que se modificara el registro
        mov ebx,[ebp+8]; le paso a ebx el valor del tope que se paso por c
        mov ecx,ebx; le asigno a ecx el valor del tope que tenia en ebx, ya que en este programa se usa ecx para eso
        pop ebx
...
;-------------------------------------------------------------------------
; sys_write: escribe al file_descriptor indicado el string apuntado por str de longitud len
;-------------------------------------------------------------------------
; Argumentos: 
; int sys_write(int file_descriptor, void* str, int len);
;-------------------------------------------------------------------------
; Devuelve el numero de bytes escritos si no hay error. Si no, devuelve -1 y errno se setea para
; indicar la causa
;-------------------------------------------------------------------------
sys_write:
    push ebp
